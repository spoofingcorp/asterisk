...
protocol=udp  
bind=0.0.0.0  
; local\_net : CRITIQUE pour le NAT. Indique à Asterisk : "Si l'IP cible est dans ce réseau,  
...
force\_rport=yes  
; rewrite\_contact=yes : Réécrit l'IP du header Contact avec l'IP source du paquet IP (vital pour les utilisateurs nomades).  
...
type=auth  
auth\_type=password  
password=ComplexPassAlice\_2025\!  
username=6001  
\[6001\]  
type=aor  
max\_contacts=2 ; Permet à Alice d'avoir PC et Mobile connectés en même temps.
...
\[commercial\]  
mode=files  
directory=/var/lib/asterisk/sounds/custom\_marketing  
...
same \=\> n,Set(CALLERID(all)="Societe ABC" \<${SDA\_STANDARD}\>)  
; \--- Compatibilité SIP Headers \---  
; Certains opérateurs (Trunk Enterprise) exigent P-Asserted-Identity (PAI) pour la facturation.  
same \=\> n,Set(PJSIP\_HEADER(add,P-Asserted-Identity)=\<sip:${SDA\_STANDARD}@sip.provider.com\>)  
...
exten \=\> ${SDA\_STANDARD},1,Goto(accueil-entreprise,s,1)
